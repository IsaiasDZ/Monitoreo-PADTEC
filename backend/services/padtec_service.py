...
class PadtecService:
    def __init__(self):
        ...
        # Con el sondeo en paralelo (varios card_id a la vez), varias
        # peticiones pueden recibir 403 al mismo tiempo; sin este lock cada
        # hilo dispararia su propio relogin simultaneo, pisandose el token
        # entre si.
    ...
    def _login(self) -> str:
        ...
        resp = self._session.post(login_url, json=payload, timeout=settings.PADTEC_TIMEOUT)
    ...
    def _ensure_token(self):
        if not self._token:
            with self._token_lock:
                if not self._token:  # doble check: otro hilo pudo loguearse mientras esperabamos
    ...
    def _get(self, url: str):
        ...
        self._ensure_token()
        resp = self._session.get(url, headers=self._headers(), timeout=settings.PADTEC_TIMEOUT)

        if resp.status_code in (401, 403):
            token_before = self._token
            with self._token_lock:
                if self._token == token_before:  # nadie lo renovo todavia -> lo hago yo
                    self._token = self._login()
